...
def on_message(client, userdata, msg):
    ...
    if msg.topic == "sensor/voltaje":
        t, v, r = payload.split(',')
        tiempo.append(int(t))
        voltaje.append(float(v))
        resistencia.append(float(r))
        datos_totales.append({'Tiempo_s': t, 'Voltaje_V': v, 'Resistencia_Ohm': r})
    elif msg.topic == "sensor/estado":
        estado_sd = payload

client = mqtt.Client()
client.on_connect = on_connect
client.on_message = on_message
client.connect("192.168.137.104", 1883, 60)  # Cambia la IP por tu broker
client.loop_start()

# --- Dash app ---
app = dash.Dash(__name__)
server = app.server  # Para desplegar en servidor si quieres
